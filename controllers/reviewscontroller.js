...
router.get("/myreviews", validateJWT, async (req, res) => {
	...
	try {
		const userReviews = await ReviewModel.findAll({
			//find all reviews from "ONE" user wherein "id" from current request matches owner id in the DB.
			where: {
				ownerID: id,
			},
		});
		...
		res.status(500).json({
			//if 500 error, return and jsonify error
			error: `[error]: ${err}`,
		});
	}
});
...
router.get("/search/:value", validateJWT, async (req, res) => {
	...
	try {
		const allReviews = await ReviewModel.findAll({
			//find all reviews from all users wherein title from current request matches title in the DB.
			where: {
				title: value,
			},
		});
